package net.zam.zamaquaticadditions.loot;
...
@Mod.EventBusSubscriber(modid = ZAMAquaticAdditions.MOD_ID)
public class ZAMLootTables {
    ...
    private static ResourceLocation register(String path) {
        return ZAMBuiltInLootTables.register(new ResourceLocation(ZAMAquaticAdditions.MOD_ID, path));
    }
        ...
        if (name.equals(net.minecraft.world.level.storage.loot.BuiltInLootTables.FISHING)) {
            ...
            if (pool != null) {
                if (ZAMConfig.LOST_OPTIONS.addLostBountyToLoot.get()) {
                    LootPoolEntryContainer neptuniumEntry = LootTableReference.lootTableReference(LOST).setWeight(1).setQuality(2).when(LootItemEntityPropertyCondition.hasProperties(LootContext.EntityTarget.THIS, EntityPredicate.Builder.entity().subPredicate(FishingHookPredicate.inOpenWater(true)))).build();
    ...
    private static void addEntry(LootPool pool, LootPoolEntryContainer entry) {
        try {
            ...
            ZAMAquaticAdditions.LOGGER.error("Error occurred when attempting to add a new entry, to the fishing loot table");
            e.printStackTrace();
        }
    }
}
