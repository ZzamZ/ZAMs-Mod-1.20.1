...
public interface ZAMCauldronRegistry extends CauldronInteraction {
    ...
    static void init() {
        EMPTY.put(Items.MILK_BUCKET, (state, level, pos, player, hand, stack) -> {
            if (!level.isClientSide()) {
                ...
                level.setBlockAndUpdate(pos, ZAMBlocks.MILK_CAULDRON.get().defaultBlockState());
        ...
        CHEESE.put(Items.SUGAR, (state, level, pos, player, hand, stack) -> {
            if (!level.isClientSide()) {
                player.awardStat(Stats.ITEM_USED.get(stack.getItem()));
                level.setBlockAndUpdate(pos, ZAMBlocks.BLUE_CHEESE_CAULDRON.get().defaultBlockState());
                player.awardStat(Stats.USE_CAULDRON);
                if (!player.isCreative()) stack.shrink(1);
                level.playSound(null, pos, ZAMSounds.BLUE_CHEESE_MADE.get(), SoundSource.BLOCKS);
                level.playSound(null, pos, ZAMSounds.CHEESE_MADE.get(), SoundSource.BLOCKS, 1.0F, 0.75F);
        ...
        CHEESE.put(Items.LAVA_BUCKET, (state, level, pos, player, hand, stack) -> {
            if (!level.isClientSide()) {
                player.awardStat(Stats.ITEM_USED.get(stack.getItem()));
                level.setBlockAndUpdate(pos, ZAMBlocks.NETHER_CHEESE_CAULDRON.get().defaultBlockState());
                player.awardStat(Stats.USE_CAULDRON);
                player.setItemInHand(hand, ItemUtils.createFilledResult(stack, player, new ItemStack(Items.BUCKET)));
                level.playSound(null, pos, ZAMSounds.NETHER_CHEESE_MADE.get(), SoundSource.BLOCKS);
                level.playSound(null, pos, ZAMSounds.CHEESE_MADE.get(), SoundSource.BLOCKS, 1.0F, 0.5F);
                level.gameEvent(null, GameEvent.BLOCK_PLACE, pos);
            } else {
                for (int i = 0; i < 10; i++) {
