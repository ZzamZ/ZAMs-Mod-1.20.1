package net.zam.zamaquaticadditions.entity.animal;
...
public class Koi extends AbstractFish {
    ...
    public void aiStep() {
        ...
        if (level().getGameTime() % 20 == 0 && ZAMConfig.COMMON.serenityEffect.get()) {
            int horizontalRange = ZAMConfig.COMMON.koiHorizontalSerenityRange.get();
            int verticalRange = ZAMConfig.COMMON.koiVerticalSerenityRange.get();
            for (Player player : level().getEntitiesOfClass(Player.class, this.getBoundingBox().inflate(horizontalRange, verticalRange, horizontalRange))) {
                if (!level().isClientSide())
                    player.addEffect(new MobEffectInstance(ZAMEffects.SERENITY.get(), 100, 0, false, false));
            }
        }
        super.aiStep();
    }
    ...
    public ItemStack getBucketItemStack() {
        return new ItemStack(ZAMItems.KOI_BUCKET.get());
    }
