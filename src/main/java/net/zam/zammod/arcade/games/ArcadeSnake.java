...
public class ArcadeSnake extends ArcadeGame {
    ...
    private static boolean highScoresUpdated = false;
    ...
    private void updateHighScores() {
        ...
        if (highScores.size() < 3) {
            isScoreHigher = true; // Less than 3 scores, automatically a high score
        } else if (score > highScores.get(highScores.size() - 1).score) {
            isScoreHigher = true; // New score is higher than the lowest in the top 3
        }
        ...
        if (isScoreHigher) {
            ...
            while (highScores.size() > 3) {
                highScores.remove(highScores.size() - 1); // Keep only the top 3 scores
            }
            highScoresUpdated = true; // Flag that high scores have changed
        }
    }

    private void displayHighScores() {
        if (highScoresUpdated) {
            int startY = 4; // Adjust based on UI layout
            s.setColors(0x8, 0xa); // Example color setup
            s.print(pw + 1, startY++, "High Scores:");
            for (HighScoreEntry entry : highScores) {
                String scoreLine = entry.playerName + ": " + entry.score;
                s.print(pw + 1, startY++, scoreLine);
            }
            highScoresUpdated = false; // Reset flag after updating display
        }
    }
}
