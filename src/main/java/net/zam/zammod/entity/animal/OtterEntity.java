package net.zam.zammod.entity.animal;
...
public class OtterEntity extends Animal {
    ...
    public ItemStack eat(Level level, ItemStack itemStack) {
        if (itemStack.is(ZAMItems.CLAM.get())) {
            if (this.random.nextFloat() <= 0.07F) {
                Vec3 mouthPos = this.calculateMouthPos();
                ItemEntity pearl = new ItemEntity(level, mouthPos.x(), mouthPos.y(), mouthPos.z(), new ItemStack(ZAMItems.OLD_RECORD_BOX.get()));
    ...
    private OtterState currentState = OtterState.IDLE;

    @Override
    public void tick() {
        super.tick();
        updateOtterState();
        performAnimationBasedOnState();
    }

    private void updateOtterState() {
        if (this.isInWater()) {
            if (this.isFloating()) {
                currentState = OtterState.FLOATING;
            } else {
                currentState = OtterState.SWIMMING;
            }
        } else if (this.isEating()) {
            currentState = OtterState.EATING;
            manageEating();
        } else if (this.onGround() && this.getDeltaMovement().lengthSqr() > 0.1) {
            currentState = OtterState.WALKING;
        } else {
            currentState = OtterState.IDLE;
        }
    }

    private void performAnimationBasedOnState() {
        switch (currentState) {
            case FLOATING:
                break;
            case SWIMMING:
    ...
    public class GoToSurfaceGoal extends Goal {
        ...
        @Override
        public void start() {
            if (OtterEntity.this.getMainHandItem().is(ZAMItems.CLAM.get())) {
